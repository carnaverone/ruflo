 ...
 *
 * The limits are intentionally conservative for plugin manifests: they prevent
 * prototype-pollution keys and resource-exhaustion shapes while accepting
 * ordinary metadata documents.
 */
  ...
  while (stack.length > 0) {
    ...
    for (let index = entries.length - 1; index >= 0; index -= 1) {
      ...
      if (RESERVED_KEYS.has(key)) {
        fail('RESERVED_KEY', `Reserved JSON key "${key}" is not allowed at ${current.path}`);
      }
      stack.push({
        value: child,
        depth: current.depth + 1,
